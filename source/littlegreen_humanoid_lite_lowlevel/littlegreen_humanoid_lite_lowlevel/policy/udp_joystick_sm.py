...
"""
#**VAPORS** Termporarily Disabled Mode Switch
...
class UdpJoystick(UdpController):
    ...
    def _update_controller(self) -> None:
        ...
        # Enter RL control mode (A + Right Bumper)
        if self.stick_states.get(StickEntry.BTN_A):
            print("buttonA pressed")
            mode_switch = 3
        # Enter init mode (A + Left Bumper)
        if self.stick_states.get(StickEntry.BTN_B):
            print("buttonB pressed")
            mode_switch = 2
        # Enter idle mode (B or Left/Right Thumbstick)
        if self.stick_states.get(StickEntry.BTN_Y):
