"""Velocity-Lilgreen-Hardware-ST3215-Loaded-v3: v1.4.3 move-now gait-pressure curriculum.

This task is additive. It preserves the v1.4.0/v1.4.2 policy interface, action
...
* stronger anti-stall / anti-double-support pressure;
* swing-foot clearance and command-aligned swing-foot motion rewards;
* a modestly faster training actuator response to test whether the previous
  delay/velocity conservatism was making the policy brace instead of step.
"""
...
@configclass
class V143ST3215LoadedFastActionsCfg:
    """Action contract v3 with a deliberately faster training response model.

    This does not change the ONNX/deployment action semantics. It only relaxes the
    training actuator-response proxy so this experiment can test whether the prior
    very conservative response model encouraged bracing instead of stepping.
    """
    ...
    joint_pos = mdp.ST3215MeasuredResidualJointPositionActionCfg(
        ...
        preserve_order=True,
        actuator_model_name=(
            f"{DATASET_NAME}:stage_a_fast_train+{LOADED_DATASET_NAME}:stage_b_loaded_fast_train"
        ),
        actuator_model_stage="stage_b_loaded_v143_fast_train",
        velocity_amplitude_knots_rad=ST3215_STEP_AMPLITUDE_KNOTS_RAD,
        velocity_curves_rad_s=_scaled_curves(ST3215_PEAK_VELOCITY_CURVES_RAD_S, 1.12),
        tau_median_s=_scaled(ST3215_TAU_MEDIAN_S, 0.78),
        tau_p10_s=_scaled(ST3215_TAU_P10_S, 0.78),
        tau_p90_s=_scaled(ST3215_TAU_P90_S, 0.78),
        ...
        velocity_scale_range=(1.04, 1.18),
        randomize_tau=True,
        ...
        loaded_velocity_scale_range=(1.04, 1.16),
        randomize_loaded_velocity_scale=True,
    )


@configclass
class HardwareV143MoveNowRewardsCfg(HardwareRewardsCfg):
    """Hardware rewards for the v1.4.3 move-now experiment."""
    ...
    moving_no_progress_l1 = RewTerm(
        ...
        params={"command_name": "base_velocity", "command_threshold": 0.12, "min_fraction": 0.35},
        weight=-1.0,
    )
...
@configclass
class ST3215LoadedV143HardwareCurriculumsCfg:
    hardware_stage = CurrTerm(func=mdp.st3215_loaded_v143_hardware_stage_curriculum)
...
@configclass
class LilgreenHardwareST3215LoadedV143EnvCfg(V140ST3215LoadedHardwareAlignedEnvCfg):
    """v1.4.3 Hardware move-now task.

    Resume this task from the successful v1.4.0 Stand model_5000.pt when testing
    whether explicit anti-bracing gait pressure plus a modestly faster actuator
    response proxy gets the robot to lift and place its feet.
    """

    commands: HardwareCommandsCfg = HardwareCommandsCfg()
    observations: HardwareObservationsCfg = HardwareObservationsCfg()
    actions: V143ST3215LoadedFastActionsCfg = V143ST3215LoadedFastActionsCfg()
    rewards: HardwareV143MoveNowRewardsCfg = HardwareV143MoveNowRewardsCfg()
    terminations: V123TerminationsCfg = V123TerminationsCfg()
    events: ST3215HardwareEventsCfg = ST3215HardwareEventsCfg()
    curriculum: ST3215LoadedV143HardwareCurriculumsCfg = ST3215LoadedV143HardwareCurriculumsCfg()
